...
est_den_ic <- function(hope_aspire_data, param_list) {
  ...
  ic_df <- aspire_df %>% dplyr::select(pid)
  ...
  l0_df <- phi2_df %>% dplyr::select(l_0)
  ...
  ic_df <- left_join(ic_df, phi2_df %>% dplyr::select(pid, ic_2), by = "pid")
  # Part 3 (Phi_3) ----------------------------------------------------------
  ## TODO: CHECK WHAT SHOULD BE DONE HERE!!!!!
  phi3_df <- hope_surv$obs
  phi3_df$q <- phi3_df$est - qfun(phi3_df %>% select(l_0))
  phi3_df <- phi3_df %>%
    mutate(ratio = risk_ratio_pred(l_0),
           dif = est - q, ic_3 = ratio * dif * (1 / prop_r))
  ic_df <- left_join(ic_df, phi3_df %>% dplyr::select(pid, ic_3), by = "pid")
  # Part 4 (Phi_4) ----------------------------------------------------------
  phi4_df <- aspr_active_surv$marg_aspr_df
  l0_df <- phi4_df %>% dplyr::select(l_0)
  ...
  # include adherence.
  ic_df <- left_join(ic_df, phi4_df %>% dplyr::select(pid, ic_4), by = "pid")
  # Part 5 (Phi_5) ----------------------------------------------------------
  ...
  phi5_df$param_ratio <-
    (1 - aspr_pl_surv(l0_df)) * (1 - hope_surv_pred(l0_df)) /
      (1 - qfun(l0_df))
  denom_val <- mean(phi5_df$param_ratio)
  phi5_df$ic_5 <- (1 / prop_r) *
    (phi5_df$param_ratio - denom_val)
  ic_df <- left_join(ic_df, phi5_df %>% dplyr::select(pid, ic_5), by = "pid")
  all_ic <- apply(ic_df %>% dplyr::select(-pid), 1, sum, na.rm = TRUE)
  return(list(ic = all_ic, est = denom_val, ic_mat = ic_df))
}
